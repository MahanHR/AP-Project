    ...
    public partial class OrderReportWin : UserControl
    {
        ...
        private void Serach_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                conn.Open();
                string c = "select * from Customer where CustomerID = '" + CustomerID + "'";
                SqlDataAdapter dadapter = new(c, conn);
                DataTable dt = new();
                dadapter.Fill(dt);
                ...
                string command = "select * from Orders where SSN = '" + dt.Rows[0][4].ToString() + "'";
                int isOneTrue = 0;
                if (price)
                {
                    if (isOneTrue == 0)
